...
export default async function convexAdminKey(cluster: string, service: string) {
  const { profile } = await import('./utils')
  ...
  async function _waitForService(
    cluster: string,
    service: string,
  ): Promise<string> {
    ...
    // Get the most recent task (assuming ARNs are ordered by creation time)
    const taskArn = response.taskArns[0]
    console.log('🔑 Task ID:', taskArn)

    return taskArn
  }

  async function _runGenerateAdminKey(
    cluster: string,
    container: string,
    task: string,
  ) {
      ...
      proc.on('close', (code) => {
        if (code === 0) {
          ...
          if (adminKeyMatch && adminKeyMatch[1]) {
            ...
            console.log(output)
            reject(new Error('Admin Key not found.'))
          }
          resolve()
        } else {
  ...
  try {
    console.log('🔑 Running script to generate admin key')
    const task = await _waitForService(cluster, service)

    await _runGenerateAdminKey(cluster, service, task)
  } catch (error) {
